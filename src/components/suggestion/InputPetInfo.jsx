...
import { StepArea, HeendyArea, ShopperHeendy, TitleBox, DescArea, InputArea, InputPetInfoBox, InputBoxes } from './suggestion.style';
...
function InputPetInfo(props) {
  ...
  const handleFormSubmit = async (event) => {
    ...
    if(selectedPet == undefined) {
      ...
      scrollToTop();
    } else if(ingredientInputRef.current == undefined || ingredientInputRef.current.files[0] == undefined) {
          ...
          return [
            {
                ...
                "price": 53000,
                "mainImgUrl": "https://heendy-feed.s3.amazonaws.com/product_4df6e4a7719a611d698b8428cb0e89974fcdb15f08e53732aa9ad07d09baa0f2.png",
                "ingredients": "뼈를 발라낸 칠면조,통건조란,완두콩,완두분,감자,감자가루,천연 맛,아마인,사과,카놀라유,뼈를 발라낸 언어,뼈를 발라낸 오리,야자유,탄산칼슘,제2인산칼슘,토마토,알팔파,당근,호박,고구마,바나나,블루베리,크랜베리,블랙베리,석류,파파야,렌즈콩,브로콜리,말린 치커리 뿌리,삼인산나트륨,염화나트륨,염화칼륨,염화콜린, 비타민(비타민 A 보충제, 비타민 D3 보충제, 비타민 E 보충제, 니코틴산, L-아스크로빌-2-폴리포스페이트, d-판토텐산 칼슘, 질산티아민, 베타카로틴, 리보플라빈, 염산 피리독신, 엽산, 비오틴, 비타민 B12 보충제),미네랄(아연 메티오닌 복합체, 아연 단백질 화합물, 철 단백질 화합물, 구리 단백질 화합물, 산화아연, 망간 단백질 화합물, 황산구리, 황산 제1철, 요오드산칼슘, 산화망간, 셀레늄 효모),타우린,DL-메티오닌,L-리진,말린 락토바실러스 아시도필러스 발효제품,말린 엔테로코커스 패시엄 발효제품,파슬리,박하,녹차 추출물,L-카르니틴,말린 로즈메리",
                "similarity": "84.29%"
            }
          ]
        });
  ...
  return (
    <>
      <DescArea>
        <HeendyArea>
          ...
          <TitleBox>
            ...
            <p className='title2'>
              우리 반려동물 기호에 딱 맞는 새로운 사료, 궁금하다면?<br/>
              <strong><mark>지금 잘 섭취하는 사료 정보</mark>를 입력</strong>하시면 <strong><mark>성분 유사도가 높은 순서</mark></strong>로 <strong>추천</strong>해드릴게요
            </p>
          </TitleBox>
        </HeendyArea>
        <StepArea>
          <Stepper className="stepper" activeStep={activeStep} alternativeLabel>
            {steps.map(label => (
              <Step key={label}>
                <StepLabel className='MuiStepLabel-label'>
                  {label}
                </StepLabel>
              </Step>
            ))}
            ...
            <InputBoxes show={recommendProduct.length != 0}>
              ...
              <div className="step-box" id="step1">
                <div className="step-text">
                  <p className={"badge " + ((activeStep === 0 && selectedPet == undefined) ? "active-bg" : "basic-bg")}>STEP 1</p>
                  <p className="step-desc">맞춤 추천을 받고 싶은, 나의 반려동물을 선택해주세요.</p>
                </div>
                    ...
                    <div key={pet.codeValue} className="pet-info">
                      <div
                        ...
                        onClick={() => {
                          if(activeStep > 0) {
                            swal.fire({
                              title: '진행중인 AI 추천이 있습니다. 초기화하시겠습니까?',
              ...
              <div className="step-box" id="step2">
                <div className="step-text">
                  <p className={"badge " + ((activeStep === 1 && (ingredientInputRef.current == undefined || ingredientInputRef.current.files[0] == undefined)) ? "active-bg" : "basic-bg")}>STEP 2</p>
                  <p className="step-desc">현재 잘 섭취하고 있는 사료가 있다면, 해당 상품 표지와 성분표를 찍어 올려주세요.</p>
                </div>
                {/* 사료 업로드 박스 */}
                <div className="feed-box">
                  ...
                  {/* 사료 성분표 이미지 */}
                  <div className="upload-section">
                    <p className="box-title"><strong className='req-title'>*</strong> 잘 먹는 사료 성분표</p>
                    <div
                      className="image-preview"
                      onClick={() => {
                        if(selectedPet == undefined) {
                    ...
                    <input
                      ...
                      onChange={(event) => {
                        const ingredientUrl = handleFileInputChange('ingredient')(event);
                          ...
                          if(activeStep == 1) {setActiveStep(2); scrollTo("step3");}
                      }}
                      className="file-input"
              ...
              <div className="step-box" id="step3">
                <div className="step-text">
                  <p className={"badge " + ((ingredientInputRef.current != undefined && ingredientInputRef.current.files[0] != undefined) ? "active-bg" : "basic-bg")}>STEP 3</p>
                  <p className="step-desc">STEP 3. 현대 더펫의 OCR 시스템과 AI 기반으로 상품 성분을 읽어, 가장 성분이 유사한 사료를 추천해드릴게요.</p>
                </div>
              </div>
            </InputBoxes>
            {
              ...
              ?
              <>              
                <div className="btn btn-custom btn-reset" onClick={() => {resetInput();}}>
                  처음부터
                </div>
                <button type="submit" form="suggestForm" className={"btn btn-custom " + ((ingredientInputRef.current != undefined && ingredientInputRef.current.files[0] != undefined) ? "active-bg" : "basic-bg")}>
                  맞춤 사료 찾기
                </button>
              </>
              :
      ...
      </InputArea>
      {
        ...
        :
        <>
          <SuggestionResult recommendProduct={recommendProduct}></SuggestionResult>    
